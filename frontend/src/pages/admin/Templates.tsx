...
const Templates: React.FC = () => {
  ...
  const handleDuplicate = async (template: Template) => {
    try {
      const duplicate = await templateService.duplicateTemplate(template.id)
      toast(`Template duplicated as "${duplicate.name}"`)
      loadTemplates()
    } catch (err: any) {
      toast(err.response?.data?.detail || 'Failed to duplicate template')
    }
  }
